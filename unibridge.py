...
class AppBase(ad.ADBase):
  ...
  def _log(self, level, prefix, message, *args):
    ...
    if l == 'DEBUG': 
      self.get_ad_api().log(msg = m, level = "INFO", log = LOG_DEBUG)
      self.api.log(msg = m, level = "INFO", log = LOG_DEBUG)
    else:
      self.get_ad_api().log(msg = m, level = l)
      self.api.log(msg = m, level = l)
...
class App(AppBase):
  ...
  def _event_callback(self, *args):
    ...
    for t in self.trigger_data:
      ...
      if t['payload_type'] in ['json']: value = payload.get(t['key'])
      else: value = payload

      if value: self._event(value)